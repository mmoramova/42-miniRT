...
void	set_cylinder(t_scene *scene, char **datarow, int *orderref)
{
	...
	cylinder->c_point = v_substr(set_vector(datarow[1]),
	...
	*orderref += 1;
	...
	set_planes_cylinder(&cylinder->upper_p, cylinder->c_upper,
		cylinder->c_direction);
	set_planes_cylinder(&cylinder->down_p, cylinder->c_down,
		v_multd(-1, cylinder->c_direction));
