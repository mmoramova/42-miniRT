...
/*   Updated: 2023/12/07 19:53:18 by josorteg         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		intersection_sphere(t_ray *ray, t_sphere *object)
{
	...
	if (t2 > 0 && (d2 < d1|| t1 < 0))
		d = d2;
	radio = vectorminus(vectoradd(escalarxvector(d,normalize_vector(u_ray)) ,ray->line.l_point),object->sp_point); //vectoradd(escalarxvector(d,u_norm), punto_recta);
